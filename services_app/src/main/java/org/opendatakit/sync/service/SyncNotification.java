...
public final class SyncNotification {
  ...
  public synchronized void updateNotification(SyncProgressState pgrState, String text,
    ...
    builder.setContentTitle(cntxt.getString(R.string.sync_notification_syncing, appName))
    ...
    notificationManager.notify(appName, UPDATE_NOTIFICATION_ID, syncNotif);
  ...
  public synchronized void finalErrorNotification(String text) {
    ...
    finalBuilder.setContentTitle(cntxt.getString(R.string.sync_notification_failure, appName))
  ...
  public synchronized void finalConflictNotification(int tablesWithProblems) {
    ...
    this.progressStatus = new SyncProgressEvent(text, SyncProgressState.FINISHED, -1, 0);
    Notification.Builder finalBuilder = new Notification.Builder(cntxt);
    finalBuilder.setContentTitle(cntxt.getString(R.string.sync_notification_conflicts, appName))
        .setContentText(text).setAutoCancel(true).setOngoing(false);
  ...
  public synchronized void clearNotification(int pendingAttachments) {
    ...
    if (pendingAttachments > 0) {
      text = cntxt.getString(R.string.sync_notification_success_pending_attachments_text,
          pendingAttachments);
      finalBuilder.setContentTitle(
          cntxt.getString(R.string.sync_notification_success_pending_attachments, appName))
          .setContentText(text).setAutoCancel(true).setOngoing(false);
      ...
      finalBuilder
          .setContentTitle(cntxt.getString(R.string.sync_notification_success_complete, appName))
          .setContentText(text).setAutoCancel(true).setOngoing(false);
      finalBuilder.setSmallIcon(R.drawable.ic_done_white_24dp);
    }
    this.progressStatus = new SyncProgressEvent(text, SyncProgressState.FINISHED, -1, 0);
  ...
  public synchronized void clearVerificationNotification() {
    ...
    finalBuilder.setContentTitle(
        cntxt.getString(R.string.sync_notification_success_verify_complete, appName))
        .setContentText(text).setAutoCancel(true).setOngoing(false);
    finalBuilder.setSmallIcon(R.drawable.ic_done_white_24dp);
    this.progressStatus = new SyncProgressEvent(text, SyncProgressState.FINISHED, -1, 0);
