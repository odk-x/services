    ...
    try {
      ...
      if (fileLock != null) {
        ...
        if (!dbFile.exists()) {
          ...
          dbConnection = attemptToOpenDb(appNameSharedStateContainer, sessionQualifier, context);
...
//          if (version == 0) {
        // CAL: This has to change based on how I need to do this for my setup
...
//        }
        ...
        ex.printStackTrace();
        logInfo(appName, "initDatabase exception: " +  ex);
      } finally {
  ...
  private OdkConnectionInterface attemptToOpenDb(AppNameSharedStateContainer appNameSharedStateContainer, String sessionQualifier, Context context)
