...
public class ODKDatabaseImplUtils {
  ...
  public OdkDbTable getRowsWithId(OdkConnectionInterface db, String tableId, String rowId) {
    OdkDbTable table = rawSqlQuery(db, OdkDbQueryUtil
        .buildSqlStatement(tableId, OdkDbQueryUtil.GET_ROWS_WITH_ID_WHERE,
            OdkDbQueryUtil.GET_ROWS_WITH_ID_GROUP_BY,
            OdkDbQueryUtil.GET_ROWS_WITH_ID_HAVING,
  ...
  private boolean hasConflictRows(String appName, OdkDbTable table) {
   ...
   public OrderedColumns getUserDefinedColumns(OdkConnectionInterface db, String appName,
    ...
    //@formatter:on
      ...
      try {
         c = db
             .query(DatabaseConstants.COLUMN_DEFINITIONS_TABLE_NAME, cols, selection, selectionArgs,
         ...
         while (!c.isAfterLast()) {
            ...
            userDefinedColumns
                .add(new Column(elementKey, elementName, elementType, listOfChildren));
            c.moveToNext();
         }
      } finally {
         if (c != null && !c.isClosed()) {
   ...
   public int getTableHealth(OdkConnectionInterface db, String tableId) {
      ...
      b.append("SELECT SUM(case when _savepoint_type is null then 1 else 0 end) as checkpoints,")
          .append(
              "SUM(case when _conflict_type is not null then 1 else 0 end) as conflicts from \"")
            ...
            Collection<File> files = FileUtils.listFiles(file, new IOFileFilter() {
               ...
               @Override public boolean accept(File file) {
                  ...
                  return (parts[0].equals(tableId) && parts[parts.length - 1].equals("csv") && (
                      parts.length == 2 || parts.length == 3 || (parts.length == 4 && parts[
                          parts.length - 2].equals("properties"))));
               }
   ...
   public void privilegedUpdateDBTableETags(OdkConnectionInterface db, String tableId,
      ...
      if (tableId == null || tableId.length() <= 0) {
         throw new IllegalArgumentException(
             t + ": application name and table name must be specified");
      }
   ...
   */
    ...
    * @return
    */
   public ArrayList<KeyValueStoreEntry> getDBTableMetadata(OdkConnectionInterface db,
   ...
   */
      ...
      for (ColumnDefinition ci : orderedDefs.getColumnDefinitions()) {
         ...
         for (int i = 0; i < ciList.size(); ++i) {
            if (!refList.contains(ciList.get(i))) {
               throw new IllegalStateException(
                   "Unexpected mis-match of listOfStringElementKeys[" + i + "] for elementKey: "
                       + ci.getElementKey());
            }
         }
         ...
         if (success == false) {
            ...
            if (columns != null) {
               ...
               if (colNames != null && colNames.length() > 0) {
                  colNames.deleteCharAt(colNames.length() - 1);
                  WebLogger.getLogger(appName).e(t,
                      "createOrOpenDBTableWithColumnsAndProperties: Error while adding table "
                          + tableId + " with columns:" + colNames.toString());
               }
            } else {
               WebLogger.getLogger(appName).e(t,
                   "createOrOpenDBTableWithColumnsAndProperties: Error while adding table "
   ...
   private void deleteServerConflictRowWithId(OdkConnectionInterface db, String tableId,
      ...
      String whereClause = String
          .format("%s = ? AND %s = ? AND %s IN ( ?, ? )", DataTableColumns.ID,
   ...
   */
   ...
   public void deleteLastCheckpointRowWithId(OdkConnectionInterface db, String appName,
      ...
      rawCheckpointDeleteDataInDBTable(db, appName, tableId, rowId,
          DataTableColumns.ID + "=? AND " + DataTableColumns.SAVEPOINT_TYPE + " IS NULL " + " AND "
              ...
              + DataTableColumns.ID + "=?)", new String[] { rowId, rowId });
   }
    ...
    * @param rowId
    */
   public void saveAsCompleteMostRecentCheckpointRowWithId(OdkConnectionInterface db,
       String tableId, String rowId) {
    ...
    * @param rowId
    */
   public void privilegedDeleteRowWithId(OdkConnectionInterface db, String appName, String tableId,
       String rowId) {
   ...
   private TableSecuritySettings getTableSecuritySettings(OdkConnectionInterface db,
      ...
      for (KeyValueStoreEntry entry : entries) {
         if (entry.key
             .equals(LocalKeyValueStoreConstants.TableSecurity.KEY_FILTER_TYPE_ON_CREATION)) {
   ...
   */
      ...
      try {
               ...
               if (shouldUpdate) {
                  if (cursor.getCount() == 1) {
                     if (cursor.moveToFirst()) {
                        int filterTypeCursorIndex = cursor
                            .getColumnIndex(DataTableColumns.FILTER_TYPE);
                        ...
                        int filterValueCursorIndex = cursor
                            .getColumnIndex(DataTableColumns.FILTER_VALUE);
                        priorFilterValue = cursor.isNull(filterValueCursorIndex) ?
                            null :
                            cursor.getString(filterValueCursorIndex);
                        int syncStateCursorIndex = cursor
                            .getColumnIndex(DataTableColumns.SYNC_STATE);
         ...
         if (update) {
            ...
            if (cvDataTableVal.containsKey(DataTableColumns.SAVEPOINT_TYPE) && (
                cvDataTableVal.get(DataTableColumns.SAVEPOINT_TYPE) == null)) {
               cvDataTableVal
                   .put(DataTableColumns.SAVEPOINT_TYPE, SavepointTypeManipulator.complete());
            }
            ...
            try {
               ...
               for (ColumnDefinition child : cp.getChildren()) {
                  ...
                  if (subcp.isUnitOfRetention()) {
                     ...
                     // next round).
                     moreToResolve.put(subkey, ODKFileUtils.mapper
                         .writeValueAsString(struct.get(subcp.getElementName())));
                  }
               }
   ...
   private static void commonTableDefn(OdkConnectionInterface db) {
      ...
      WebLogger.getLogger(db.getAppName())
          .i("commonTableDefn", DatabaseConstants.UPLOADS_TABLE_NAME);
      ...
      db.execSQL(ColumnDefinitionsColumns
          .getTableCreateSql(DatabaseConstants.COLUMN_DEFINITIONS_TABLE_NAME), null);
      WebLogger.getLogger(db.getAppName())
          .i("commonTableDefn", DatabaseConstants.KEY_VALUE_STORE_ACTIVE_TABLE_NAME);
      db.execSQL(KeyValueStoreColumns
          .getTableCreateSql(DatabaseConstants.KEY_VALUE_STORE_ACTIVE_TABLE_NAME), null);
      WebLogger.getLogger(db.getAppName())
